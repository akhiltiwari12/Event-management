...
const Task = () => {
  ...
  useEffect(()=>{
    const fetch=async()=>{
        const response=await axios.get(`https://event-green.vercel.app/api/v1/task/${id}/alltasks`,{
  ...
  try {
    await axios.delete(`https://event-green.vercel.app/api/v1/task/${id}/delete/${index}`,{
        withCredentials:true,
        headers:{
            "Content-Type":"application/json",
        }
  ...
  const handleStatusChange = async (index, newStatus) => {
    try {
        await axios.put(`https://event-green.vercel.app/api/v1/task/${id}/status/${index}`,{
            status:newStatus
        },{
            withCredentials:true,
            headers:{
